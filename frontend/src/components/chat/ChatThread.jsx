...
const ChatThread = ({ conversationId, otherUser }) => {
    const { user } = useAuth();
    ...
    if (!conversationId) {
        return (
            <div className="flex-1 flex flex-col items-center justify-center text-gray-500 bg-white/5 backdrop-blur-xl">
    ...
    return (
        <div className="flex-1 flex flex-col h-full bg-[#0a0a14] border-l border-white/10 relative overflow-hidden">
            {/* Header */}
            <div className="p-4 border-b border-white/10 bg-white/5 backdrop-blur-md flex items-center gap-4">
                <div className="w-10 h-10 rounded-full overflow-hidden bg-amber-500/10 border border-white/10">
                ...
                <div>
                    <h3 className="text-white font-bold">{otherUser?.username || 'Usuario'}</h3>
            ...
            {/* Input Area */}
            <form onSubmit={handleSend} className="p-4 bg-white/5 backdrop-blur-md border-t border-white/10 flex gap-3">
                <input 
                    type="text" 
                    value={input}
                ...
                />
                <button type="submit" className="w-12 h-12 bg-amber-500 rounded-xl flex items-center justify-center text-black hover:bg-amber-400 transition-all shadow-lg active:scale-95">
